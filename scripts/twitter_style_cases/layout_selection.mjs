...
export function buildPagePlan(style, spec, index, previousLayout) {
  const pageOverride =
    spec.id === "page-08" ? "evidence-quote"
    : spec.id === "page-05" && style.family === "minimal" ? "editorial-thesis"
    : null;

  const familyChoice = familyLayoutMap[style.family]?.[spec.role];
  const familyCandidates = [pageOverride, familyChoice, ...spec.preferredLayouts].filter(Boolean);
